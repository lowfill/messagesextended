...
//TODO Manage case when a recipient is indicated in advance
if (is_plugin_enabled("libform_utils") && get_plugin_setting('multiple', 'messagesextended') == "true") {
	$recipient_drop_down = elgg_view('input/autosuggest', array(
			'name' => "recipient_guid",
			'suggest'=>'messages_users',
			'style'=>'-facebook'));
}
elseif (elgg_is_active_plugin('autocomplete')) {
	...
	if(empty($recipient_guid)){
		...
		if(!empty($group_options)){
			$group_options[''] = elgg_echo('messages:choose:group');
			$group_drop_down = elgg_view('input/dropdown', array(
					'name' => 'group_guid',
					'value' => '',
